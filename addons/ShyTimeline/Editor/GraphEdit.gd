...
var node_types := []
var timeline: TimelineRes
var _connect_to := {}


func _ready() -> void:
	add_child(node_menu)
	node_menu.connect("id_pressed", _on_menu_item_pressed)
...
func _on_copy_nodes_request() -> void:
	...
	var json = JSON.new()
	OS.clipboard = json.stringify(data)
...
func _create_node(event: Resource) -> GraphNode:
	...
	node.connect("request_name_change", _on_node_change_name_request, [node])
	node.connect("slot_updated", _on_node_slot_updated, [node])
	node.connect("close_request", _delete_node, [node])
	node.event = event
	return node
