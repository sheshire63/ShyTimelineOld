...
func format_text(text: String, replace_new_lines := false, add_wait := true) -> Array:
	...
	for i in lines.size():
		if lines[i].has("text"):
			...
			while regex_mach:
				var tmp: String = lines[i].text
				lines[i].text = tmp.left(regex_mach.get_start())
				lines[i].text = tmp.right(regex_mach.get_start() + 1)
...
func _handle_bracket(line: String, opt: bool) -> Array:
	...
	var random = r.randf_range(0.0, max(total_weight, 1.0))
	for j in weights.size():
		random -= weights[j]
		if random <= 0.0:
...
#			pass#handeld in _handle_action
		...
		match action:
			...
			"*":
				if value_a is String:
					value_a = (value_a as String).repeat(int(value_b))
				elif value_a is int and value_b is int:
					value_a = value_a * value_b
				else:
					value_a = float(value_a) * float(value_b)
			...
			"<":
				if value_a is String:
					if value_b is String:
						value_a = (value_a as String).length() < value_b.length()
					else:
			...
			"<>":
				if value_a is String:
					...
					value_a = float(value_a) != float(value_b)
			"<<":
				value_a = (value_a as int) << int(value_b)
			">>":
		...
		if action != "" and actions[0].get_string("var") == "":
			...
			match operator:
				...
				"/=":
					variables[variable] /= value
				"**=":
					variables[variable] = pow(variables[variable], value as float)
				"//=":
					variables[variable] = pow(variables[variable], 1 / (value as float))
		else:
			result.append_array(format_text(str(_add_up_values(actions, opt), opt, false)))
		result.append(entry)
		actions.pop_front()
	return result
