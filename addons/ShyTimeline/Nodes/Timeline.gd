...
func _ready() -> void:
	if Engine.editor_hint:
		return
	connect("handle_event", _default_handler)
...
func _process(_delta: float) -> void:
	...
	if event_queue.is_empty() and active_events.is_empty():
...
func _unhandled_key_input(input_event: InputEventKey) -> void:
	if input_event.is_action_pressed("ui_up"):
		if history:
			...
			var do = true
			while do and not history.is_empty():
	...
	if input_event.is_action_pressed("ui_down"):
		if future:
			...
			var do = true
			while do and not future.is_empty():
