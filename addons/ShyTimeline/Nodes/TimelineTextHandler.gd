...
@export_node_path(RichTextLabel) var text_label_path
...
func _process(_delta: float) -> void:
	if Engine.editor_hint:
		return
	if current_event.is_empty() and !is_active:
		if _segments.size() <= _segment:
			_finish_handle()
		else:
			await _handle_segment(_segments[_segment], current_event.event)
	elif not _queue.is_empty() and (interrupt or !is_active):
...
func _finish_handle() -> void:
	if _future.is_empty() and not current_event.is_empty():
			...
			"wait", "w", "end_line":
				if segment.value != "":
					...
					if get_setting("auto"):
						...
						await _continue #add timer for auto
			"continue", "c", "cont":
				if segment.value != "":
	...
	if _history:
		...
		if _history and _history[-1].id == id:
			...
			for i in event.segments:
				await _handle_segment(i, id, true)
			...
			if fast:
				for i in item.segments.size():
					if _future or i < _segment:
						await _handle_segment(item.segments[i], event.event, true if _future.size() > 1 else false)
					else:
						current_event = item
			else:
				_segment = 0
