...
@export var button_overrides := []#fix nodepath array
@export_node_path(Container) var button_container_path
@export var wait_for_button_press := true
@export_node_path(Range) var time_bar_path
	...
	if event.get_event_type() == "Choice":
		...
		for i in event.choice_text.size():
			...
			buttons[i].get_node((button_text_property as NodePath)).set(
					...
					text)
			buttons[i].connect("pressed", _on_button_pressed, [i + 1, event_id])
...
func _reset_buttons() -> void:
	for i in button_overrides:
		if i:
			i = get_node(i)
			if i.is_connected("pressed", _on_button_pressed):
				i.disconnect("pressed", _on_button_pressed)
			i.visible = false
